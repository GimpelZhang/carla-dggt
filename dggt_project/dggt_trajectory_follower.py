...
This provides maximum control for following DGGT trajectories that may deviate
...
class DggtTrajectoryFollower:
    ...
    def __init__(self, dggt_actor, world: carla.World):
        """
        Initialize the DGGT trajectory follower.

        Args:
            dggt_actor: DggtActor instance containing the vehicle and track data
            world: CARLA World instance
        """
        self.dggt_actor = dggt_actor
        self.world = world
        self.vehicle = dggt_actor.actor_inst
        self.track = dggt_actor.track
        ...
        logger.debug(f"Initialized DGGT trajectory follower for vehicle {self.vehicle.id}")
    ...
    def set_trajectory_from_track(self, start_time: int, end_time: int, time_spacing: int = 50000) -> None:
        """
        Extract trajectory points directly from DGGT track data.
    ...
    def get_lookahead_point(self, current_scenario_time: float) -> Optional[carla.Transform]:
        ...
        前瞻时间根据当前速度动态调整：低速时看得近（精准过弯），高速时看得远（提前变道）。
        ...
        dynamic_lookahead_time = max(0.5, min(2.0, current_speed_ms * 0.1))
        lookahead_time_us = dynamic_lookahead_time * 1_000_000  # Convert to microseconds
        # =====================================================================
