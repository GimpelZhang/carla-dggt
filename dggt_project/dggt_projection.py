...
"""
DGGT Projection and Coordinate Transformation Functions Module

This module provides functions for coordinate system transformations between different
reference frames used in geospatial and mapping applications for DGGT integration.
...
- get_t_rig_enu_from_ecef: Align DGGT scene and OpenDRIVE coordinate systems using georeference data

The module is essential for aligning DGGT scene reconstructions with OpenDRIVE maps, as they
may have different coordinate system origins and orientations. The transformations ensure
proper spatial alignment between the scene data and the map data.
...
def lat_lng_alt_2_ECEF_elipsoidal(lat_lng_alt: np.ndarray, a: float, b: float) -> np.ndarray:
    """
    Converts the GPS (lat, long, alt) coordinates to ECEF based on the ellipsoidal earth model.

    Args:
        lat_lng_alt (np.ndarray): latitude, longitude and altitude coordinate (in degrees and meters) [n,3]
        a (float): Semi-major axis of the ellipsoid (WGS84: 6378137.0 m)
        b (float): Semi-minor axis of the ellipsoid

    Returns:
        np.ndarray: ECEF coordinates [n,3]
    """
...
def ecef_2_ENU(loc_ref_point: np.ndarray, earth_model: str = "WGS84") -> np.ndarray:
    """
    Compute the transformation matrix that transforms points from ECEF to a local ENU coordinate frame.
    ...
    Args:
        ...
        earth_model: Earth model used for conversion. "WGS84" for ellipsoidal (accurate for large maps),
                     "sphere" for spherical model (less accurate for large-scale scenarios)

    Returns:
        T_ecef_enu: 4x4 transformation matrix from ECEF to ENU
    """
    ...
    if earth_model == "WGS84":
        ...
        raise ValueError(f"Selected ellipsoid '{earth_model}' not implemented!")
...
def get_t_rig_enu_from_ecef(t_rig_ecef: np.ndarray, xodr_data: str) -> np.ndarray:
    ...
    This function is essential for aligning the coordinate systems used by DGGT and the OpenDRIVE map.
    The DGGT system and the OpenDRIVE map may have different origins and orientations, so this function
    ...
    if len(lats) != 1 or len(lons) != 1 or len(alts) != 1:
        print(f"Unable to parse proj string! {geo_reference.text}")
        return np.identity(4)
    ...
    # Check if translation is larger than 10 km (sanity check for alignment)
