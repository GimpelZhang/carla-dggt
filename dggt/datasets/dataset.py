...
class WaymoOpenDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 1:
            ...
            indices = [current_start + i * self.interval for i in range(self.sequence_length)]
            intervals = [self.interval for _ in range(self.sequence_length - 1)]
            
            # timestamps = np.array(indices) - start_idx
            timestamps = np.array(indices) - current_start
            timestamps = timestamps / timestamps[-1] * (self.sequence_length / 4)
            if self.views == 3:
                timestamps = np.repeat(timestamps, 3)
            ...
            indices = [current_start + i * self.interval for i in range(self.sequence_length)]
            intervals = [self.interval for _ in range(self.sequence_length - 1)]
            target_indices = [current_start + i for i in
                              range(self.sequence_length * self.interval - (self.interval - 1))]
